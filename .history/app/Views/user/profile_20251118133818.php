  ...
  <div class="flex-1 flex flex-col">
    ...
    <div class="bg-white flex items-center justify-start px-20 py-16 w-full relative">
      ...
      <div class="relative w-64 h-64 rounded-full overflow-hidden bg-gray-800 flex items-center justify-center mr-16">
        <img id="previewImage" src="<?= esc($profile['foto']) ?>" alt="Profile"
      ...
      <div class="text-left">
        <div class="space-y-2 text-xl">
          <div><span class="font-semibold">Nama:</span> <?= esc($profile['nama']) ?></div>
          <div><span class="font-semibold">Email:</span> <?= esc($profile['email']) ?></div>
          <div><span class="font-semibold">Telpon Number:</span> <?= esc($profile['telp']) ?></div>
        </div>
      </div>
    </div>
    ...
    <div class="bg-[#1A1A1A] text-white px-20 py-10 w-full">
      ...
      <div class="bg-white text-black rounded-xl p-6 overflow-x-auto">
        <table class="w-full text-left border-collapse">
          ...
          <tbody>
            <?php foreach ($pesanan_saat_ini as $p): ?>
              <tr class="border-b hover:bg-gray-100 transition">
                <td class="p-3"><?= esc($p['nama']) ?></td>
                <td class="p-3"><?= esc($p['layanan']) ?></td>
                <td class="p-3 text-yellow-600 font-medium"><?= esc($p['waktu']) ?></td>
                <td class="p-3"><?= esc($p['tanggal']) ?></td>
                <td class="p-3 font-semibold <?= $p['status'] === 'Dibatalkan' ? 'text-red-500' : 'text-green-500' ?>">
                  <?= esc($p['status']) ?>
                </td>
              </tr>
            <?php endforeach; ?>
    ...
    <div class="bg-[#1A1A1A] text-white px-20 py-10 w-full flex-1">
      ...
      <div class="bg-white text-black rounded-xl p-6 overflow-x-auto">
        <table class="w-full text-left border-collapse">
          ...
          <tbody>
            <?php foreach ($riwayat_pesanan as $r): ?>
              <tr class="border-b hover:bg-gray-100 transition">
                <td class="p-3"><?= esc($r['nama']) ?></td>
                <td class="p-3"><?= esc($r['layanan']) ?></td>
                <td class="p-3 text-yellow-600 font-medium"><?= esc($r['waktu']) ?></td>
                <td class="p-3"><?= esc($r['tanggal']) ?></td>
                <td class="p-3 text-yellow-600 font-semibold"><?= esc($r['status']) ?></td>
              </tr>
            <?php endforeach; ?>
          </tbody>
        </table>
